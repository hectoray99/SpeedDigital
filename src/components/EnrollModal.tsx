...
export default function EnrollModal({ isOpen, onClose, studentId, studentName, onSuccess }: Props) {
    const { orgData } = useAuthStore(); // Usamos la memoria global para evitar consultas extra
    ...
    async function fetchProducts() {
        try {
            const { data, error } = await supabase
                .from('catalog_items')
                .select('*')
                .eq('organization_id', orgData?.id) // BLINDAJE: Solo planes de este local
                .eq('is_active', true)
    ...
    // =========================================================================
    // PROCESO DE INSCRIPCIÓN Y GENERACIÓN DE DEUDA
    // =========================================================================
    const handleEnroll = async () => {
        if (!selectedProductId || !studentId || !orgData?.id) return;
        setLoading(true);
        ...
        try {
            ...
            if (personFetchError || !person) throw new Error('Alumno no encontrado');

            let currentPlans = person.details?.active_plans || [];

            // Retrocompatibilidad para alumnos creados en versiones viejas del sistema
            ...
            // --- 3. ACTUALIZAR EL PERFIL DEL ALUMNO ---
            ...
            // --- 4. GENERAR DEUDA (OPERACIÓN PENDIENTE) ---
            const { data: op, error: opError } = await supabase
                ...
                .insert({
                    ...
                    balance: product.price, // Debe todo el monto inicial
            ...
            // Línea de detalle de la operación
                                    ...
                                    <div className="space-y-3 max-h-64 overflow-y-auto pr-2 hide-scrollbar">
                                        {products.length === 0 ? (
                                            ...
                                            products.map((product) => (
                                                <label
                                                    key={product.id}
                                                    className={`flex flex-col p-4 rounded-2xl border-2 cursor-pointer transition-all relative overflow-hidden group ${
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-start justify-between mb-3">
                                                        <div className="flex items-start gap-3">
                                                            <input
                                                                ...
                                                                className="w-4 h-4 mt-1 text-brand-600 border-slate-300 focus:ring-brand-500 cursor-pointer"
                                                            />
                                                            <div>
                                                    ...
                                                    <div className="pl-7 flex flex-wrap items-center gap-2 text-[11px] font-bold text-slate-500 uppercase tracking-wide">
