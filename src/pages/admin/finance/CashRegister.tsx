...
import { Wallet, Lock, Unlock, ArrowUpFromLine, Loader2, DollarSign, ReceiptText, TrendingUp, TrendingDown, X, AlertTriangle, ShoppingCart, Scale } from 'lucide-react';
...
export default function CashRegister() {
    ...
    const handleExpenseMovement = async () => {
        ...
        if (!movementNotes.trim()) return toast.error("Debes justificar el retiro");
    ...
    // --- NUEVA LÓGICA DE CIERRE Y AJUSTE DE CAJA ---
    const handleCloseRegister = async () => {
        if (!closeAmount || isNaN(Number(closeAmount))) return toast.error("Ingresá el dinero real contado");

        const realAmount = Number(closeAmount);
        const diff = realAmount - expectedTotal;

        // Confirmación si hay diferencias (arqueo)
        if (diff !== 0) {
            const msg = diff > 0 
                ? `Hay un SOBRANTE de $${diff.toLocaleString()}. Se creará un movimiento de ajuste. ¿Confirmar cierre?` 
                : `Hay un FALTANTE de $${Math.abs(diff).toLocaleString()}. Se creará un retiro de ajuste. ¿Confirmar cierre?`;
        ...
        setIsSaving(true);
        try {
            // 1. Si hay diferencia, creamos el movimiento en el Libro de Finanzas para blanquearlo
            if (diff !== 0) {
            ...
            // 2. Cerramos el turno en la tabla de cajas
            const { error } = await supabase
                .from('cash_sessions')
                .update({
                    status: 'closed',
    ...
    if (loading) return <div className="p-16 flex flex-col items-center justify-center text-slate-400"><Loader2 className="w-10 h-10 animate-spin text-brand-500 mb-4" /><p className="font-bold">Calculando saldos...</p></div>;
        ...
        return (
            <div className="max-w-2xl mx-auto mt-8 md:mt-16 px-4 animate-in fade-in zoom-in-95 duration-500">
                <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 text-center flex flex-col items-center relative overflow-hidden">
                    ...
                    <p className="text-slate-500 mb-10 font-medium">Abrí el turno ingresando el dinero inicial para empezar a vender y cobrar.</p>
                    ...
                    {isOpening ? (
                        <div className="w-full max-w-sm animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <div className="relative mb-6 shadow-sm rounded-2xl">
                                ...
                                <input type="number" value={openingAmount} onChange={(e) => setOpeningAmount(e.target.value)} placeholder="Ej: 5000" autoFocus className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-brand-500/10 focus:border-brand-400 focus:bg-white font-black text-2xl outline-none transition-all text-slate-800" />
                            </div>
                            <div className="flex gap-3">
                                ...
                                <button onClick={handleOpenRegister} className="flex-[2] py-4 rounded-xl font-black text-white bg-brand-600 hover:bg-brand-500 shadow-lg shadow-brand-500/20 transition-all active:scale-95">Abrir Turno</button>
                            </div>
                        </div>
                    ) : (
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="bg-white p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm flex flex-col justify-between relative overflow-hidden">
                    ...
                    <p className="text-slate-400 font-bold text-xs uppercase tracking-widest mb-2 flex items-center justify-between">
                        Ingresos <span className="text-emerald-500 bg-emerald-50 px-2 py-0.5 rounded-md">+{movements.filter(m => m.type === 'income').length}</span>
                    </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                    ...
                    <div className="flex-1 overflow-y-auto p-3 md:p-4 hide-scrollbar">
                        {movements.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-slate-400">
                                ...
                                <p className="font-bold">Aún no hay ventas en este turno.</p>
                            </div>
                        ) : (
                                ...
                                {movements.map((m) => (
                                    <div key={m.id} className="flex items-center justify-between p-4 hover:bg-slate-50 rounded-2xl transition-colors border border-transparent hover:border-slate-100 group">
                                        <div className="flex items-center gap-4">
                                            <div className={`p-3 rounded-xl shadow-sm border ${m.type === 'income' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-red-50 text-red-500 border-red-100'}`}>
                                        ...
                                        <p className={`font-black text-xl px-4 py-2 rounded-xl bg-white border shadow-sm ${m.type === 'income' ? 'text-emerald-600 border-emerald-100' : 'text-red-500 border-red-100'}`}>
                ...
                <div className="space-y-4">
                    <button onClick={() => setIsSaleModalOpen(true)} className="w-full bg-brand-600 hover:bg-brand-500 text-white p-6 rounded-3xl shadow-xl shadow-brand-500/20 transition-all flex items-center justify-center flex-col gap-3 group active:scale-95">
                        ...
                        <div>
                            <h3 className="font-black text-2xl mb-1">Terminal de Venta</h3>
                            <p className="text-brand-200 text-sm font-medium">Facturar en mostrador</p>
                        </div>
                    </button>

                    <button onClick={() => setIsMovementModalOpen(true)} className="w-full bg-white border border-slate-200 hover:border-red-300 p-6 rounded-3xl shadow-sm hover:shadow-md hover:shadow-red-500/10 transition-all flex items-center gap-5 group">
                        ...
                        <div className="text-left">
                            <h3 className="font-black text-slate-800 text-xl mb-1">Retiro de Efectivo</h3>
                            <p className="text-sm font-medium text-slate-500">Pagar a proveedores</p>
                        </div>
                    </button>
                </div>
            </div>
                    ...
                    <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div className="flex gap-3 pt-2">
                                <button onClick={() => setIsMovementModalOpen(false)} className="flex-1 py-4 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">Cancelar</button>
                                <button onClick={handleExpenseMovement} disabled={isSaving} className="flex-[2] py-4 rounded-xl font-black text-white shadow-lg transition-all active:scale-95 bg-red-500 hover:bg-red-400 shadow-red-500/30">
                                    {isSaving ? "Guardando..." : "Confirmar Retiro"}
                                </button>
                            </div>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-6 space-y-6">
                            <div className="bg-slate-900 text-white p-6 rounded-2xl flex flex-col items-center justify-center shadow-inner relative overflow-hidden">
                                <div className="absolute top-0 right-0 w-24 h-24 bg-brand-500 rounded-full mix-blend-screen filter blur-[40px] opacity-50"></div>
                            ...
                            <div>
                                ...
                                <div className="relative shadow-sm rounded-xl">
                                    <DollarSign className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-6 h-6" />
                                    <input type="number" value={closeAmount} onChange={(e) => setCloseAmount(e.target.value)} placeholder="Ej: 110000" autoFocus className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-brand-500/10 focus:border-brand-400 focus:bg-white font-black text-3xl outline-none transition-all text-slate-800" />
                                </div>
                                
                                {/* FEEDBACK VISUAL DEL ARQUEO */}
                                {closeAmount !== '' && (
                                    <div className={`p-3 rounded-xl mt-3 text-sm font-bold text-center border animate-in fade-in duration-300 ${
                            ...
                            <div className="flex gap-3 pt-2">
                                <button onClick={() => setIsCloseModalOpen(false)} className="flex-1 py-4 rounded-xl font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">Cancelar</button>
                                <button onClick={handleCloseRegister} disabled={isSaving || !closeAmount} className="flex-[2] py-4 rounded-xl font-black text-white bg-slate-900 hover:bg-slate-800 shadow-xl shadow-slate-900/20 transition-all active:scale-95 disabled:opacity-50">
                                    {isSaving ? "Procesando..." : "Cerrar Turno"}
                                </button>
                            </div>
                        </div>
                    </div>
