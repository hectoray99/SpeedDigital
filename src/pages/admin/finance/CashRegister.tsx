...
import { Wallet, Lock, Unlock, ArrowUpFromLine, Loader2, DollarSign, ReceiptText, TrendingUp, TrendingDown, X, AlertTriangle, ShoppingCart, Scale, Info, Hash, User } from 'lucide-react';
...
export default function CashRegister() {
    ...
    async function fetchMovements() {
        if (!orgData?.id || !activeSession) return;
    ...
    const handleOpenRegister = async () => {
        ...
        if (amount > 99999999) return toast.error("El monto excede el límite permitido.");
    ...
    const handleExpenseMovement = async () => {
        ...
        if (amount > 99999999) return toast.error("Monto fuera del rango.");
        if (!movementNotes.trim()) return toast.error("La justificación es obligatoria.");
    ...
    const handleCloseRegister = async () => {
        ...
        if (closeAmount === '' || isNaN(realAmount)) return toast.error("Debes ingresar el dinero físico.");
        if (realAmount < 0) return toast.error("El monto no puede ser negativo.");
        ...
        if (diff !== 0) {
            const msg = diff > 0 
                ? `¡ATENCIÓN! Hay un SOBRANTE de $${diff.toLocaleString()}. El sistema creará un ajuste automático. ¿Confirmar cierre?` 
                : `¡ATENCIÓN! Hay un FALTANTE de $${Math.abs(diff).toLocaleString()}. El sistema creará un ajuste automático. ¿Confirmar cierre?`;
    ...
    return (
        <div className="max-w-6xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-12">
            ...
            {/* EL MODAL DE VENTA AHORA ESTÁ LIBERADO DE LA CAJA */}
            <CreateOperationModal 
                isOpen={isSaleModalOpen} 
                onClose={() => setIsSaleModalOpen(false)} 
                onSuccess={() => { if (activeSession) fetchMovements(); }} 
            />

            {loading ? (
                <div className="p-16 flex flex-col items-center justify-center text-slate-400 animate-in fade-in">
                    <Loader2 className="w-10 h-10 animate-spin text-brand-500 mb-4" />
                    <p className="font-bold tracking-widest text-sm uppercase">Calculando saldos...</p>
                </div>
            ) : !activeSession ? (
                // ==========================================
                // VISTA: CAJA CERRADA
                    ...
                    <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 border border-slate-100 text-center flex flex-col items-center relative overflow-hidden">
                        ...
                        {isOpening ? (
                            <div className="w-full max-w-sm animate-in fade-in slide-in-from-bottom-4 mt-6">
                                {lastClosingAmount !== null && (
                                    <div className="mb-6 bg-blue-50 text-blue-700 p-3 rounded-xl text-sm font-medium flex gap-3 text-left">
                                        ...
                                        <p>La caja anterior se cerró con <b>${lastClosingAmount.toLocaleString()}</b>. Modificalo si hiciste algún retiro/depósito intermedio.</p>
                                    </div>
                                )}
                            ...
                            <div className="w-full mt-2">
                                <p className="text-slate-500 mb-8 font-medium">Podés iniciar la jornada para llevar el control estricto de dinero en efectivo, o ir directamente al facturador si no manejás caja física.</p>
                                <div className="flex flex-col sm:flex-row gap-4 w-full justify-center">
                                    <button onClick={() => setIsOpening(true)} className="bg-slate-900 hover:bg-slate-800 text-white px-6 py-4 rounded-2xl font-black text-lg shadow-xl shadow-slate-900/20 transition-all flex items-center justify-center gap-3 active:scale-95 flex-1">
                                        <Unlock className="w-6 h-6" /> Iniciar Jornada
                                    </button>
                                    <button onClick={() => setIsSaleModalOpen(true)} className="bg-brand-600 hover:bg-brand-500 text-white px-6 py-4 rounded-2xl font-black text-lg shadow-xl shadow-brand-500/30 transition-all flex items-center justify-center gap-3 active:scale-95 flex-1 border-b-4 border-brand-800">
                                        <ShoppingCart className="w-6 h-6" /> Terminal Venta
                                    </button>
                                </div>
                            </div>
                        )}
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-2 mb-1">
                                                            ...
                                                            {m.operations?.number && (
                                                                <span className="text-[10px] font-black uppercase tracking-widest bg-brand-50 text-brand-600 border border-brand-100 px-1.5 py-0.5 rounded flex items-center gap-0.5">
                                                                    <Hash className="w-3 h-3" /> {m.operations.number}
                                                                </span>
                                                            )}
                                                        </div>
                                                        
                                                        <div className="flex flex-wrap items-center gap-2 text-xs font-bold text-slate-400 tracking-wide">
                                                            <span>{new Date(m.processed_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                                                            <span>•</span>
                                                            <span className="uppercase text-slate-500">{m.payment_method}</span>
                                                            
                                                            {m.crm_people?.full_name && (
                                                                <>
                                                                    <span>•</span>
                                                                    <span className="flex items-center gap-1 text-slate-600 bg-slate-100 px-1.5 py-0.5 rounded-md">
                                                                        <User className="w-3 h-3" /> {m.crm_people.full_name}
                                                                    </span>
                                                                </>
                                                            )}
                                                        </div>
                        ...
                        <div className="space-y-4">
                            <button onClick={() => setIsSaleModalOpen(true)} className="w-full bg-brand-600 hover:bg-brand-500 text-white p-6 rounded-3xl shadow-xl shadow-brand-500/20 transition-all flex items-center justify-center flex-col gap-3 group active:scale-95 border-b-4 border-brand-800">
